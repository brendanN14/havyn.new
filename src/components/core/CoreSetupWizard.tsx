  ...
  const handleCreateDemoData = async () => {
    ...
    try {
      ...
      const { data: property, error: propertyError } = await supabase
        ...
        .insert({
          user_id: user.id,
          name: 'Demo Property - Sunset Apartments',
      ...
      if (propertyError) {
        logError('Creating demo property', propertyError);
        return;
      }

      // Create 20 demo units
      const demoUnits = [];
      for (let floor = 1; floor <= 4; floor++) {
        for (let unit = 1; unit <= 5; unit++) {
          const unitCode = `${floor}0${unit}`;
          const isOccupied = Math.random() > 0.4; // 60% occupied
          demoUnits.push({
            property_id: property.id,
            unit_code: unitCode,
            beds: unit <= 2 ? 1 : 2,
            baths: unit <= 2 ? 1 : unit === 5 ? 2 : 1,
            sqft: unit <= 2 ? 650 : unit === 5 ? 950 : 850,
            asking_rent: unit <= 2 ? 1200 : unit === 5 ? 1700 : 1500,
      ...
      if (unitsError) {
        logError('Creating demo units', unitsError);
        return;
      }

      setCreatedPropertyId(property.id);
      setPropertyForm({ ...propertyForm, name: property.name });
      setUnitsCreated(20);
      setCurrentStep('complete');
    } catch (err: any) {
              ...
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center">
                  ...
                  <span className={`mt-2 text-xs font-medium ${
                    isActive || isComplete ? 'text-havyn-primary' : 'text-gray-400'
                  }`}>
                    {step.label}
                  </span>
                </div>
                {index < steps.length - 1 && (
                  <div className={`w-16 h-0.5 mx-2 mb-6 ${
                    index < currentStepIndex ? 'bg-havyn-primary' : 'bg-gray-200 dark:bg-gray-700'
                  }`} />
                )}
              </React.Fragment>
            );
